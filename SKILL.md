...
优先保证 Results 与 Discussion；Methods 从简；减少正文与补充材料间的重复。
...
- **图**：作为独立文件提交；正文须引用每张图并按出现顺序编号 (Figure_1, Figure_2…)；每图有
  ...
  （单栏 ≥1063 px，整页 ≥2244 px）；位图线条图 ≥1000 dpi。颜色对色觉障碍者友好。
- **表**：可编辑文本（非图片）；正文引用并按序编号；表注置于表下；避免竖线与底纹；少用表、
  不与正文数据重复。
...
7. 全程冲突一律按 §0 优先级裁决；Nature 风格绝不凌驾 Food Chemistry。
