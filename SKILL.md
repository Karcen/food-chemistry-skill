...
优先保证 Results 与 Discussion；Methods 从简；减少正文与补充材料间的重复。注意组图与排版，组图建议写代码进行组图，或者使用无边框表格进行排版，优先级：写代码组图 > 使用无边框表格进行排版。
...
- **图**：作为独立文件提交；正文须引用每张图并按出现顺序编号 (Figure_1, Figure_2…)；每图有
  ...
  （单栏 ≥1063 px，整页 ≥2244 px）；位图线条图 ≥1000 dpi。颜色对色觉障碍者友好。下文若提及图如根据图X，则使用蓝色超链接交叉引用。
